...
function FileContain({ text }) {
  return (
    <div style={{backgroundColor:"#F0F0F0", borderRadius:"8px", marginBottom:"10px", padding:"3px 10px 3px 10px", display:"inline-block", marginRight:"15px"}}>
      <img src={paperclip} style={{width:"20px", height:"20px", marginRight:"10px", marginTop:"2px", marginBottom:"5px",display:"inline-block"}}/>
      <p style={{display:"inline-block", marginBottom:"0px", fontSize:"15px"}}>{text}</p>
    </div>
  );
}


function InputStuff({email, subject, body, active}){
  ...
  const handleFileChange = (event) => {
    const newSelectedFiles = [...selectedFiles, ...Array.from(event.target.files)];
  ...
  return (
    <div style={{alignItems:'left', textAlign:"left"}}>
      ...
      <input id="emailInput" type="text" value={emailN} style={{width:"84vw", color:"#727272", border:"0px", borderRadius:"8px", fontSize:"15px", padding:"10px 10px 10px 10px"}} onChange={handleEmail} disabled={!active}/>
      
      <h3 style={{textAlign:"left", fontSize:"20px", marginTop:"20px"}}>Subject</h3>
      <input id="emailInput" type="text" value={subjectN} style={{width:"84vw", color:"#727272", border:"0px", borderRadius:"8px", fontSize:"15px", padding:"10px 10px 10px 10px"}} onChange={handleSubject} disabled={!active}/>
      ...
      <label id="kev" style={{backgroundColor:"#F0F0F0", borderRadius:"8px", marginBottom:"0px", padding:"3px 0px 3px 10px", display:"flex", width:"170px"}} disabled={!active}>
        <img src={plus} style={{width:"20px", height:"20px", marginRight:"10px", marginTop:"2px"}}/>
        <p style={{fontSize:"14px", height:"7px", marginTop:"2px"}}>Add More Files</p>
        <input
         disabled={!active}
          type="file"
          hidden
      ...
      <div style={{display:"flex", marginTop:"15px", alignItems:"right", marginRight:"50px",justifyContent: "flex-end"}}>
            <button style={{backgroundColor:"transparent", color:"#102DE9", fontWeight:"bold", paddingTop:"5px", paddingBottom:"5px"}} onClick={handleCancel} disabled={!active}>Cancel</button>
            <button style={{marginLeft:"10px", backgroundColor:"#102DE9", paddingTop:"5px", paddingBottom:"5px"}} onClick={handleSend} disabled={!active}>Send</button>
      </div>
...
function App() {
  ...
  const handleFileChange = (event) => {
    ...
    /* Checking Amount of Emails, delete later*/
  ...
  setSendEmail('Kevin123@gmail.com');
  console.log("OAHWDOIAWDOI")
  setSubject("King of The World Emails");
  setBody("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE");
